...
setwd("C:/Users/Deepe/OneDrive/Desktop/Pine Breed DE - 2021")
...
generate_AMatrix <- function(selection_list,reference_list,progress) {
  ...
  progress$set(message = "Running OMM Part 1", value = 0)
  ...
  str5 <- calculate_coefficients(selection_list_A, "Selection File")
  ...
  HTML(paste(estr, str1strong, estr, str2, str3, str4, estr, str5, strtitlestrong, estr, str0, str000strong, str00, estr, sep='<br/>'))
}

#my function to calcualte coefficients - inbreeding and coancestry
...
#function to add optimized EBV in the output file
optimize_EBV <- function (file1, file2)
{
  preoptimized_output <- read.csv(file1)
  optimized_output <- preoptimized_output
  
  SD <- sqrt(5)
  for (i in 1:nrow(preoptimized_output))
  {
    ebv <- preoptimized_output$Index[i]
    max_ebv <- ebv
    for (j in 1:30)
    {
      ebv_generated <- qnorm(runif(1), mean=ebv, sd=SD)
      if (ebv_generated > ebv)
        max_ebv <- ebv_generated
    }
    optimized_output$MaxIndex[i] <- max_ebv
  }
  write.csv(optimized_output, row.names=FALSE, file = output_filename_optimized)
  
}

#function to generate input file for 2nd round
generate_input_file_next_round <- function (file1, file2)
{
  prior_input <- read.csv(file1)
  #change maxuse of previous input file to 0
  prior_input$MaxUse = 0
  
  optimized_output <- read.csv(file2)
  optimized_output$MaxUse <- 1
  optimized_output$Index <- optimized_output$MaxIndex
  
  new_input <- cbind(optimized_output$CandidateID, optimized_output$Parent1, optimized_output$Parent2)
  new_input <- cbind(new_input, optimized_output$Index)
  new_input <- cbind(new_input, optimized_output$MaxUse)
  
  colnames(new_input) <- c("Selection","Parent1","Parent2","Index","MaxUse")
  prior_input <- rbind(prior_input, new_input)
  
  write.csv(prior_input, row.names=FALSE, file = next_round_file_name)
}

#function to find if the male and female are related or not
#arguments - name of the male, name of the female, A matrix of whole progeny
...
#calculate msi for the final mating_list
...
print_output_part2 <- function (output_filename, duration)
{
  output_data <- read.csv(output_filename, check.names = FALSE)
  ...
  crosses <- length(output_data$Index)
  ...
  logfile_text <- paste(nstr, str11, nstr, str1, str16, nstr, str12, nstr, str13, nstr, str14, nstr, str15, nstr, str2, nstr, str3, nstr, str4, nstr, strtitle, nstr, str0, nstr, str5, nstr)
  ...
  HTML(paste(estr, str11strong, estr, str1, str16, estr, str12, str13, str14, str15, estr, str2, str3, str4, estr, strtitlestrong, estr, str0, str00, estr, str5strong, estr, sep='<br/>'))
}

#shinyApp UI in Dashboard style
ui <- dashboardPage(
  dashboardHeader(title = "OMM"),
  dashboardSidebar(
    sidebarMenu(
  ...
  dashboardBody(
    ...
    tabItems(
      ...
      tabItem(tabName = "part1",
        ...
        fluidRow(
          box(
            title = "Optimal Mating Software for Monoecious Species by Khushi Goda",
            width = 12 
          )
        )
      ),
     ...
     # Second tab content
      tabItem(tabName = "part2",
        ...
        fluidRow(
          box(
            actionButton("close2", "Exit", align="right"),
            width = 12
          )
        ),
        fluidRow(
     ...
     tabItem(tabName = "part3",
             fluidRow(
               ...
               box (
                 ...
                 actionButton("download_graph1", "Download"),
                 width = 4
               ),
               box (
                 h3("Mean Coancestry and Inbreeding"),
                 plotOutput("contents3_2"),
                 actionButton("download_graph2", "Download"),
                 width = 4
               )
             ),
             ...
             fluidRow(
               box(
                 actionButton("close3", "Exit", align="right"),
                 width = 12
               )
             ),
             fluidRow(
...
server <- function(input, output) {
  ...
  part1_reactive <- eventReactive(input$part1_button,{
    #output$contents1 <- renderUI({
      ...
      progress$set(message = "Running OMM Part 1", value = 0)
      generate_AMatrix(selection_list,reference_list,progress)
  ...
  part2_reactive <- eventReactive(input$part2_button,{
    #output$contents2 <- renderUI({
      ...
      print_output_part2(output_filename, as.numeric(duration))
  ...
  part3_reactive_2 <- eventReactive(input$part3_button,{
    #output$contents3_2 <- renderPlot({
      ...
      inco <- data.frame(List = c("Candidate list","Mating list"),
                         ...
                         Coefficient = c(.002,0.001,.0025,.0009))
  ...
  observeEvent(input$download_graph1, {
    ggsave("Density.png", glo_plot1)
  })
  
  observeEvent(input$download_graph2, {
    ggsave("Index.png", glo_plot2)
  })
}
